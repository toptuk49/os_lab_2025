...
int main(int argc, char **argv) {
    ProgramArguments args = {.seed = -1, .array_size = -1, .process_count = -1, .use_files = false, .timeout = -1};
    ...
    for (int process_index = 0; process_index < args.process_count; process_index++) {
        ...
        if (child_pid == CHILD_PROCESS) {
            ...
            return 0;
        } else if (child_pid == PROCESS_ERROR) {
            perror("Fork failed");
            return 1;
        }
    }
    ...
    if (timeout_flag) {
        for (int process_index = 0; process_index < args.process_count; process_index++) {
            if (kill(pipes[process_index][READ_DESCRIPTOR], CHILD_PROCESS) == STILL_ALIVE) {
...
bool ParseArguments(int argc, char **argv, ProgramArguments *arguments) {
    ...
    if (arguments->seed <= 0 || arguments->array_size <= 0 || arguments->process_count <= 0 || arguments->timeout < 0) {
        printf("Usage: --seed <num> --array_size <num> --pnum <num> [--by_files] [--timeout <num>]\n");
        return false;
    }
...
struct MinMax CollectResultsFromChildren(int process_count, bool use_files, int pipes[][2]) {
    ...
    for (int process_index = 0; process_index < process_count; process_index++) {
        ...
        if (use_files) {
            ...
            read(pipes[process_index][READ_DESCRIPTOR], &local_result, sizeof(local_result));
