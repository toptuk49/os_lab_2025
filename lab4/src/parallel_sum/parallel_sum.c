...
int main(int argc, char **argv) {
  ProgramArguments programArguments = {.seed = -1, .array_size = -1, .threads_num = -1};
    
  if (!ParseArguments(argc, argv, &programArguments)) return 1;
  ...
  for (int i = 0; i < programArguments.threads_num; i++) {
    ...
    args[i].end = (i == programArguments.threads_num - 1) ? programArguments.array_size : (i + 1) * chunk_size;
...
bool ParseArguments(int argc, char **argv, ProgramArguments *arguments) {
    ...
    if (arguments->seed <= 0 || arguments->array_size <= 0 || arguments->threads_num <= 0) {
